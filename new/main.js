...
const projectData = {
  ...
  anatomy: {
    ...
    images: [
      { src: 'images/anatomy-logo.png', alt: 'Lead Adapt Impact — University of Nottingham Physiotherapy' }
    ],
    liveLink: 'https://xerte.nottingham.ac.uk/play_53902#page1',
  ...
  deterioration: {
    tag: 'Generative AI · Consequence-Based',
    ...
    body: `<p>St Barnabas provided a Word document outlining a single patient case — the raw clinical detail, with no interactive structure. I turned it into a four-part narrative following "Bob," using generative AI to help dramatise his deterioration across the four parts.</p>
    <p>Deliberately, I steered away from testing recall with traditional multiple-choice questions. Instead, each checkpoint plays out as a small scenario: the learner makes a decision about Bob's care and lives with a realistic consequence before the story continues. The aim is for that consequence to sit at the back of the learner's mind for the rest of the module — not just register as right or wrong in the moment, but shape how they read everything that happens to Bob afterwards.</p>`,
    tech: ['Generative AI', 'Four-Part Narrative', 'Consequence-Based Scenarios', 'Word-to-Interactive Conversion']
  },
  ...
  arthur: {
    ...
    images: [
      ...
      { src: 'images/best-interests-branching-map.png', alt: 'Branching map of the Best Interests conversation scenario', caption: 'The full branching map — every path either builds trust, damages it, or ends the meeting early' }
    ],
...
const lifecycleData = {
  ...
  stbarnabas: [
    ...
    { icon:'fa-code', title:'Development',
      text:`The Skills Network's EQUAL platform doesn't support traditional SCORM authoring tools, so rather than build in an authoring tool, the modules were hand-coded natively in HTML5, React.js and CSS — with a custom xAPI (Tin Can) wrapper built directly into the code, sending proper verbs and activity statements so completion and progress track correctly through the LMS. That included diagnosing and fixing a handful of reporting mismatches during EQUAL's integration testing, before WCAG accessibility checks and functional QA ran ahead of release.`,
      points:['Native HTML5 / React.js / CSS build','Custom xAPI (Tin Can) implementation, not an authoring-tool export','Diagnosed & fixed LMS reporting mismatches during integration','WCAG accessibility checks + functional QA'] },
  ...
  telehealth: [
    ...
    { icon:'fa-rocket', title:'Launch & Evaluation',
      text:`The resource was integrated directly into the University's LMS, making it accessible within the existing curriculum with no disruption to how students already learned — including troubleshooting SCORM playback and completion-tracking issues that surfaced during rollout, so the timeline held. It went on to become an important precursor to Virtual Placement, proving out the same experiential, practice-oriented design principles later scaled into that platform.`,
      points:['Integrated into the University LMS','Troubleshot SCORM playback & completion-tracking issues','Precursor to Virtual Placement'] }
  ],
